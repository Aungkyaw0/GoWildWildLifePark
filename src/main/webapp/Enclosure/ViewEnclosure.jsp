...
<script>
  function confirmDelete(encID) {
       return confirm("Are you sure you want to delete Enclosure ID " + encID + "?");
  }
</script>
<body>
<nav>
...
<%
  try {
    ...
    ArrayList<Enclosure> enclosuresArr = eMgr.fetchAll();
...
<%
	if(ranking != null && ranking.equals("Senior")){
		...
		<% 
		 for (Enclosure enclosure : enclosuresArr) {
			  %>
			  <tr>
			    <td><%= enclosure.getEnclosure_ID() %></td>
			    <td><%= enclosure.getEnclosure_Type() %></td>
			    <td><%= enclosure.getEnclosure_Location() %></td>
			    <td><a href="EditEnclosure.jsp?EnclosureID=<%= enclosure.getEnclosure_ID() %>" id="edit">Edit</a></td>    
			    <td><a href="../Delete.jsp?InpID=<%= enclosure.getEnclosure_ID() %>&tableName=Enclosures&cName=Enclosure_ID" id="delete" onclick="return confirmDelete('<%=enclosure.getEnclosure_ID()%>');">Delete</a></td> 
			  </tr> 
		<%
		}
